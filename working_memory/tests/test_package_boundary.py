...
import ayyo_working_memory


class PackageBoundaryTest(unittest.TestCase):
    def setUp(self) -> None:
        self.repository_root = Path(__file__).resolve().parents[2]
        self.package_root = self.repository_root / "working_memory"
        self.source_root = self.package_root / "src"

    def test_core_has_no_ros_gazebo_persistence_network_or_process_dependency(self) -> None:
        forbidden = {
            "ayyo_memory",
            "builtin_interfaces",
            "gazebo",
    ...
    def test_public_exports_are_importable(self) -> None:
        self.assertTrue(ayyo_working_memory.__all__)
        for name in ayyo_working_memory.__all__:
            with self.subTest(name=name):
                self.assertTrue(hasattr(ayyo_working_memory, name))

    def test_wheel_contains_only_working_memory_runtime(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            copy = root / "working_memory"
            wheels = root / "wheels"
            ...
            self.assertTrue(all(name.startswith("ayyo_working_memory/") for name in names if name.endswith(".py")))
